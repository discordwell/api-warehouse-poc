"""
Simplified Parallel Sequencer

A working implementation of parallel sequencing without full Raft complexity.
Uses a primary-backup model where the first replica is always leader.

For production, swap in full Raft. This demonstrates the architecture.
"""
...
from ..types import Transaction, TxnId, Timestamp
...
class SimpleSequencer:
    """
    A single partition sequencer (simplified - no Raft).

    In production, this would use Raft for HA.
    """
    ...
    def _close_epoch(self):
        """Close the current epoch and commit batch."""
        with self._lock:
            epoch = self._current_epoch
            ...
            # Store batch
            self._committed_batches[epoch] = batch

            # Signal waiters
            if epoch in self._batch_events:
                self._batch_events[epoch].set()
...
class SimpleParallelSequencerCluster:
    """
    Cluster of parallel sequencers.

    Each partition has one sequencer. In production, would have replicas with Raft.
    """
    ...
    def __init__(
        ...
        # Create sequencers
        self.sequencers: Dict[int, SimpleSequencer] = {}
        for p in range(num_partitions):
            ...
            self.sequencers[p] = seq

        # Epoch assembly
        self._current_epoch = 0
        self._epoch_batches: Dict[int, Dict[int, SequencedBatch]] = {}
    ...
    def submit(self, txn: Transaction) -> int:
        """Submit a transaction for sequencing."""
        # Determine partitions touched
        partitions = self._get_touched_partitions(txn)

        epoch = None
        for p in partitions:
            epoch = self.sequencers[p].submit(txn)

        return epoch if epoch is not None else self._current_epoch

    def _get_touched_partitions(self, txn: Transaction) -> Set[int]:
        """Get partitions touched by transaction."""
        partitions = set()
